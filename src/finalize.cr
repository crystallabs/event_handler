  ...
  {% begin %}
    {% for e in Event.subclasses %}
      ...
      private def internal_insert(type : {{e.id}}.class, handler : Proc({{e.id}}, Bool), once : Bool, async : Bool, at : Int)
        ...
        _emit AddHandlerEvent, type, handler_obj.unsafe_as(::EventHandler::Handler(Proc(Event, Bool)))
      end
      ...
      # :ditto:
      def off(type : {{e.id}}.class, handler : ::EventHandler::Handler(Proc(Event, Bool)))
        if _event_{{event_name}}.delete handler
         _emit RemoveHandlerEvent, type, handler.unsafe_as(::EventHandler::Handler(Proc(Event, Bool)))
        end
      end
